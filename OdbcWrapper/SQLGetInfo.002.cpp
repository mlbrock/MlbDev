...
struct GetInfoTypeDescriptorValue {
	...
	virtual ~GetInfoTypeDescriptorValue();

	bool operator < (const GetInfoTypeDescriptorValue &other) const;

	GetInfoTypeDescriptorValue &swap(GetInfoTypeDescriptorValue &other);
...
// ////////////////////////////////////////////////////////////////////////////
GetInfoTypeDescriptorValue::GetInfoTypeDescriptorValue()
	:info_type_(0)
	,is_string_(false)
	,is_small_(false)
	,is_mask_(false)
...
GetInfoTypeDescriptorValue::GetInfoTypeDescriptorValue(
	const GetInfoTypeDescriptorRaw &raw_descriptor)
...
bool GetInfoTypeDescriptorValue::operator < (
	const GetInfoTypeDescriptorValue &other) const
{
	return(info_type_ < other.info_type_);
}

GetInfoTypeDescriptorValue &GetInfoTypeDescriptorValue::swap(
	GetInfoTypeDescriptorValue &other)
{
...
std::string &GetInfoTypeDescriptorValue::ToString(
...
std::string GetInfoTypeDescriptorValue::ToString() const
{
...
std::string &GetInfoTypeDescriptorValue::ToStringPadded(
	std::string &out_string) const
{
	if (is_string_)
		out_string.assign(value_string_);
...
std::string GetInfoTypeDescriptorValue::ToStringPadded() const
{
	std::string out_string;
...
// ////////////////////////////////////////////////////////////////////////////
struct GetInfoTypeDescriptor : public GetInfoTypeDescriptorValue {
	         GetInfoTypeDescriptor();
	explicit GetInfoTypeDescriptor(
		const GetInfoTypeDescriptorRaw &descriptor_raw,
		const GetInfoTypeDescriptorValue &descriptor_val =
		GetInfoTypeDescriptorValue());
	//	Used for find()...
	explicit GetInfoTypeDescriptor(SQLUSMALLINT info_type);
	virtual ~GetInfoTypeDescriptor();

	bool operator < (const GetInfoTypeDescriptor &other) const;

	GetInfoTypeDescriptor &swap(GetInfoTypeDescriptor &other);
...
// ////////////////////////////////////////////////////////////////////////////
GetInfoTypeDescriptor::GetInfoTypeDescriptor()
	:GetInfoTypeDescriptorValue()
{
}
...
GetInfoTypeDescriptor::GetInfoTypeDescriptor(
	const GetInfoTypeDescriptorRaw &descriptor_raw,
	const GetInfoTypeDescriptorValue &descriptor_val)
	:GetInfoTypeDescriptorValue(descriptor_val)
	,info_type_name_(descriptor_raw.info_type_name_)
	,data_type_name_(descriptor_raw.data_type_name_)
{
}

GetInfoTypeDescriptor::GetInfoTypeDescriptor(SQLUSMALLINT info_type)
	:GetInfoTypeDescriptorValue()
{
...
bool GetInfoTypeDescriptor::operator < (
	const GetInfoTypeDescriptor &other) const
{
	return(info_type_ < other.info_type_);
}

GetInfoTypeDescriptor &GetInfoTypeDescriptor::swap(GetInfoTypeDescriptor &other)
{
	GetInfoTypeDescriptorValue::swap(other);
...
std::string &GetInfoTypeDescriptor::ToString(std::string &out_string) const
{
	std::string(info_type_name_ + "=" +
		GetInfoTypeDescriptorValue::ToString()).swap(out_string);

	return(out_string);
}

std::string GetInfoTypeDescriptor::ToString() const
{
	std::string out_string;
...
std::string &GetInfoTypeDescriptor::ToStringPadded(
	std::string &out_string, std::streamsize padding_width) const
{
	...
	o_str
		...
		<<	std::right << ": "
		<< GetInfoTypeDescriptorValue::ToStringPadded(tmp_string);
...
// ////////////////////////////////////////////////////////////////////////////
class GetInfoTypeDescriptorList {
protected:
	typedef std::set<GetInfoTypeDescriptor> MySet;
	typedef MySet::const_iterator           MySetIterC;
	typedef MySet::iterator                 MySetIter;

	MySetIter FindInternal(SQLUSMALLINT info_type)
	{
		return(descriptor_set_.find(GetInfoTypeDescriptor(info_type)));
	}

	MySetIterC FindInternal(SQLUSMALLINT info_type) const
	{
		return(descriptor_set_.find(GetInfoTypeDescriptor(info_type)));
	}
	...
	MySet descriptor_set_;

public:
	GetInfoTypeDescriptorList()
		:descriptor_set_()
	{
	}
	virtual ~GetInfoTypeDescriptorList()
	{
	}

	GetInfoTypeDescriptorList &swap(GetInfoTypeDescriptorList &other)
	{
	...
	GetInfoTypeDescriptor &Get(SQLUSMALLINT info_type,
		GetInfoTypeDescriptor &out_datum) const
	{
		MySetIterC iter(FindInternal(info_type));

		if (iter == descriptor_set_.end())
			ThrowNotLocated(info_type);

		return(GetInfoTypeDescriptor(*iter).swap(out_datum));
	}
	GetInfoTypeDescriptor  Get(SQLUSMALLINT info_type) const
	{
		GetInfoTypeDescriptor out_datum;

		return(Get(info_type, out_datum));
	}

	void InsertOrUpdate(const GetInfoTypeDescriptor &new_item)
	{
	...
	static GetInfoTypeDescriptorList &Create(SQLHDBC ConnectionHandle,
		GetInfoTypeDescriptorList &out_datum)
	{
		GetInfoTypeDescriptorList tmp_datum;

		unsigned int count_1;

		for (count_1 = 0; count_1 < GetInfoTypeRawCount; ++count_1) {
			if (!GetInfoTypeRawList[count_1].is_handle_2way_) {
				GetInfoTypeDescriptor info_value;
				GetInfo(odbc_dbc_sptr->GetNativeHandle(),
					GetInfoTypeRawList + count_1, info_value);
...
namespace {
// ////////////////////////////////////////////////////////////////////////////
const GetInfoTypeDescriptorRaw GetInfoTypeRawList[] = {
...
// ////////////////////////////////////////////////////////////////////////////
unsigned int GetInfoTypeRawData(GetInfoTypeDescriptorRaw *&list_ptr)
{
...
API_ODBCWRAPPER SQLRETURN GetInfo(SQLHDBC ConnectionHandle,
	const GetInfoTypeDescriptorRaw *InfoType,
	GetInfoTypeDescriptor &InfoValuePtr,
	OdbcThrowFlags::Flags throw_flags = OdbcThrowFlags::Ok_SWI);
API_ODBCWRAPPER SQLRETURN GetInfo(SQLHDBC ConnectionHandle,
	SQLUSMALLINT InfoType, GetInfoTypeDescriptorValue &InfoValuePtr,
	OdbcThrowFlags::Flags throw_flags = OdbcThrowFlags::Ok_SWI);
API_ODBCWRAPPER SQLRETURN GetInfo(SQLHDBC ConnectionHandle,
	SQLUSMALLINT InfoType, GetInfoTypeDescriptor &InfoValuePtr,
	OdbcThrowFlags::Flags throw_flags = OdbcThrowFlags::Ok_SWI);
...
SQLRETURN GetInfo(SQLHDBC ConnectionHandle,
	const GetInfoTypeDescriptorRaw *InfoType,
	GetInfoTypeDescriptor &InfoValuePtr, OdbcThrowFlags::Flags throw_flags)
{
	SQLRETURN                  return_code;
	GetInfoTypeDescriptorValue tmp_value;

	return_code = GetInfo(ConnectionHandle, InfoType, tmp_value, throw_flags);

	GetInfoTypeDescriptor(*InfoType, tmp_value).swap(InfoValuePtr);
