...
#include <OSSupport/OSSupport_File.hpp>
...
{
	if (!IsOpen())
		MLB::Utility::ThrowInvalidArgument("Specified file handle is not open.");
}
//	////////////////////////////////////////////////////////////////////////////
...
{
	if (IsOpen())
		MLB::Utility::ThrowInvalidArgument("Specified file handle is not "
			"closed.");
}
//	////////////////////////////////////////////////////////////////////////////
...
{
	if (!IsWritable())
		MLB::Utility::ThrowException("Specified file is not writable.");
}
//	////////////////////////////////////////////////////////////////////////////
...
void OSFile::CheckOffsetFile(FileOffsetType file_offset) const
{
	...
	if (file_offset >= file_size_)
		MLB::Utility::ThrowInvalidArgument("Specified file offset " +
			MLB::Utility::AnyToString(file_offset) + " within file '" +
			file_name_ + "' is invalid because the size of the file is only " +
			MLB::Utility::AnyToString(file_size_) + " bytes.");
}
//	////////////////////////////////////////////////////////////////////////////
	...
	FileSizeType extent_length) const
{
	try {
		...
		MLB::Utility::Rethrow(except, "Specified file extent (extent starting "
			"offset " + MLB::Utility::AnyToString(extent_offset) + " +  extent "
			"length " + MLB::Utility::AnyToString(extent_length) + " = extent "
			"ending offset " + MLB::Utility::AnyToString(
			GetExtentOffsetEnd(extent_offset, extent_length)) +
	...
	bool do_not_create)
{
	...
	if ((native_handle_ = ::CreateFile(file_name.c_str(),
		...
		NULL,
		(read_only_flag || do_not_create) ? OPEN_EXISTING : OPEN_ALWAYS,
...
{
	FileSeekOffsetType return_value = 0;
	...
	if (::SetFilePointerEx(native_handle_, tmp_offset, &tmp_return_value,
		seek_method) != 0)
		return(tmp_return_value.QuadPart);
#else
	...
	MLB::Utility::ThrowSystemError("Attempt to seek within file '" +
		GetFileName() + "' to offset " + MLB::Utility::AnyToString(file_offset) +
		" using seek method " + MLB::Utility::AnyToString(seek_method) +
		" failed");
	...
	unsigned int block_length)
{
	...
	if (::ReadFile(native_handle_, block_ptr, block_length,
		&return_value, NULL) == 0)
		MLB::Utility::ThrowSystemError("Attempt to read file '" + GetFileName() +
			"' for " + MLB::Utility::AnyToString(block_length) + " bytes failed");
	...
	unsigned int block_length)
{
	...
	if (block_length != return_value)
		MLB::Utility::ThrowException("Attempt to read file '" + GetFileName() +
			"' for " + MLB::Utility::AnyToString(block_length) + " bytes failed "
			"because the actual number of bytes read was " +
			MLB::Utility::AnyToString(return_value) + ".");
	...
	unsigned int block_length)
{
	...
	if (::WriteFile(native_handle_, block_ptr, block_length,
		&return_value, NULL) == 0)
		MLB::Utility::ThrowSystemError("Attempt to write file '" +
			GetFileName() + "' for " + MLB::Utility::AnyToString(block_length) +
			" bytes failed");
	...
	unsigned int block_length)
{
	...
	if (block_length != return_value)
		MLB::Utility::ThrowException("Attempt to write file '" + GetFileName() +
			"' for " + MLB::Utility::AnyToString(block_length) + " bytes failed "
			"because the actual number of bytes written was " +
			MLB::Utility::AnyToString(return_value) + ".");
	...
	try {
		...
		if (new_file_size > current_file_size) {
			...
			if ((mmap_handle = ::CreateFileMapping(
				native_handle_,
				NULL,
				(is_read_only_) ? PAGE_READONLY : PAGE_READWRITE,
				tmp_length.HighPart,
				tmp_length.LowPart,
				NULL)) == NULL)
				MLB::Utility::ThrowSystemError("Attempt to increase the file size "
					"with 'CreateFileMapping()' failed");
			::CloseHandle(mmap_handle);
		}
		else if (new_file_size < current_file_size) {
/*
	CODE NOTE: Obsoleted, replaced with ::SetFilePointerEx(). To be removed.
			ULARGE_INTEGER  tmp_length;
			tmp_length.QuadPart = new_file_size;
			LONG            distance_low  = tmp_length.LowPart;
			LONG            distance_high = tmp_length.HighPart;
			if (::SetFilePointer(
				native_handle_,
				distance_low,
				&distance_high,
				FILE_BEGIN) == 0)
				MLB::Utility::ThrowSystemError("Attempt to prepare for a decrease "
					"in the file size with by setting the file pointer using "
					"'SetFilePointer()' failed (low distance = " +
					MLB::Utility::AnyToString(tmp_length.LowPart) + ", high "
					"distance = " + MLB::Utility::AnyToString(tmp_length.HighPart) +
					").");
			if (::SetEndOfFile(native_handle_) == 0)
				MLB::Utility::ThrowSystemError("Attempt to decrease the file size "
					"with 'SetEndOfFile()' failed");
*/
			...
			if (::SetFilePointerEx(
				...
				MLB::Utility::ThrowSystemError("Attempt to prepare for a decrease "
					...
					MLB::Utility::AnyToString(new_file_size) + ").");
			if (::SetEndOfFile(native_handle_) == 0)
				MLB::Utility::ThrowSystemError("Attempt to decrease the file size "
					"with 'SetEndOfFile()' failed");
		}
		...
		MLB::Utility::Rethrow(except, "Unable to change the size of file '" +
			file_name_ + "' from " + MLB::Utility::AnyToString(current_file_size) +
			" bytes to the requested size of " +
			MLB::Utility::AnyToString(new_file_size) + " bytes: " +
			std::string(except.what()));
	}
...
void OSFile::Flush()
{
	...
	if (::FlushFileBuffers(native_handle_) == 0)
		MLB::Utility::ThrowSystemError("Attempt to flush buffer contents for "
...
FileSizeType OSFile::GetFileSize(const std::string &file_name)
{
	...
	if ((size_low = ::GetFileSize(native_handle_, &size_high)) ==
		INVALID_FILE_SIZE)
		MLB::Utility::ThrowSystemError("Attempt to determine the file size "
			"for '" + file_name + "' failed --- GetFileSize() did not succeed");
	...
	FileSizeType granularity)
{
	...
	if (!new_file_size)
		MLB::Utility::ThrowInvalidArgument("The specified new file size "
			"(" + MLB::Utility::AnyToString(new_file_size) + ") is zero.");
	...
	TruncateToSize(MLB::Utility::GranularRoundUp(new_file_size, granularity));
}
//	////////////////////////////////////////////////////////////////////////////
...
void OSFileMMap::CheckMapped() const
{
	...
	if (!IsMapped())
		MLB::Utility::ThrowException("Specified file is not memory-mapped.");
}
//	////////////////////////////////////////////////////////////////////////////
...
{
	if (IsMapped())
		MLB::Utility::ThrowException("Specified file is memory-mapped.");
}
//	////////////////////////////////////////////////////////////////////////////
...
void OSFileMMap::CheckOffsetMMap(FileOffsetType file_offset) const
{
	...
	if (file_offset < mmap_offset_)
		MLB::Utility::ThrowInvalidArgument("Specified memory-mapped offset " +
			MLB::Utility::AnyToString(file_offset) + " within file '" +
			file_name_ + "' is invalid because the memory-mapped area begins at "
			"file offset " + MLB::Utility::AnyToString(mmap_offset_) + ".");
	else if ((file_offset - mmap_offset_) >= mmap_length_)
		MLB::Utility::ThrowInvalidArgument("Specified memory-mapped offset " +
			MLB::Utility::AnyToString(file_offset) + " within file '" +
			file_name_ + "' is invalid because the memory-mapped area extends "
			"from file offset " + MLB::Utility::AnyToString(mmap_offset_) +
			" through file offset " +
			MLB::Utility::AnyToString((mmap_offset_ + mmap_length_) - 1) +
			" inclusive.");
}
//	////////////////////////////////////////////////////////////////////////////
	...
	FileSizeType extent_length) const
{
	try {
		...
		MLB::Utility::Rethrow(except, "Specified memory-mapped extent (extent "
			"starting offset " + MLB::Utility::AnyToString(extent_offset) +
			" +  extent length " + MLB::Utility::AnyToString(extent_length) +
			" = extent ending offset " + MLB::Utility::AnyToString(
			GetExtentOffsetEnd(extent_offset, extent_length)) +
			") within file '" + file_name_ + "' memory-mapped area of length " +
			MLB::Utility::AnyToString(mmap_length_) + " is invalid: " +
			std::string(except.what()));
	}
}
//	////////////////////////////////////////////////////////////////////////////
...
void OSFileMMap::CheckGranularity(FileSizeType granularity)
{
	if (granularity % os_granularity_)
		MLB::Utility::ThrowException("Specified memory-mapping granularity (" +
			MLB::Utility::AnyToString(granularity) + ") is not an integral "
			"multiple of the operating system memory-mapping granularity (" +
			MLB::Utility::AnyToString(os_granularity_) + ").");
}
//	////////////////////////////////////////////////////////////////////////////
...
void OSFileMMap::MapFile(FileSizeType map_length, FileOffsetType map_offset)
{
	try {
		GetFileSize();
		if (!file_size_)
			MLB::Utility::ThrowInvalidArgument("The size of the file to be "
				"mapped (" + MLB::Utility::AnyToString(file_size_) + ") is zero.");
		if (map_offset >= file_size_)
			MLB::Utility::ThrowInvalidArgument("The specified mapping offset (" +
				MLB::Utility::AnyToString(map_offset) + ") is not less than the "
				"file size (" + MLB::Utility::AnyToString(file_size_) + ").");
		if (map_length > GetMaxMapLength())
			MLB::Utility::ThrowInvalidArgument("The specified mapping length (" +
				MLB::Utility::AnyToString(map_length) +
				") is greater than the maximum permissible value (" +
				MLB::Utility::AnyToString(GetMaxMapLength()) + ").");
		...
		if ((mmap_handle_ = ::CreateFileMapping(
			...
			tmp_length.LowPart,
			NULL)) == NULL)
			MLB::Utility::ThrowSystemError("Attempt to create a file mapping of "
				"size " + MLB::Utility::AnyToString(new_map_length) + " for "
				"file '" + file_name_ + "' with 'CreateFileMapping()' failed");
//		file_size_ = adj_map_length;		//	True under Windows, not Unix.
		...
		if ((mmap_ptr_ = static_cast<char *>(::MapViewOfFile(
			mmap_handle_,
			(is_read_only_) ? FILE_MAP_READ : FILE_MAP_WRITE,
			tmp_offset.HighPart,
			tmp_offset.LowPart,
			...
			if (MLB::Utility::GetLastSystemErrorCode() == ERROR_NOT_ENOUGH_MEMORY)
				throw MLB::Utility::ExceptionMMapVMFailure(error_text.str() + ": " +
					MLB::Utility::GetSystemErrorString());
			else
		...
		MLB::Utility::Rethrow(except, "Unable to map file '" + file_name_ +
			"': " + std::string(except.what()));
	}
}
//	////////////////////////////////////////////////////////////////////////////
...
void OSFileMMap::MFlush()
{
	...
	if (mmap_ptr_ != NULL) {
		if (::FlushViewOfFile(mmap_ptr_, static_cast<SIZE_T>(mmap_length_)) == 0)
			MLB::Utility::ThrowSystemError("Attempt to flush memory-mapped "
				"contents for file '" + file_name_ + "' failed.");
	}
}
// ////////////////////////////////////////////////////////////////////////////
...
{
	return(MLB::Utility::GranularRoundUp(granularity,
...
{
	return(MLB::Utility::GranularRoundUp(granularity, GetOSPageGranularity()));
}
// ////////////////////////////////////////////////////////////////////////////
	...
	FileOffsetType &map_offset, FileOffsetType &ptr_adjust)
{
	if (!map_length)
		MLB::Utility::ThrowInvalidArgument("The specified mapping length (" +
			MLB::Utility::AnyToString(map_length) + ") is zero.");
...
{
	return(static_cast<FileSizeType>(MLB::Utility::GetPageAllocGranularitySize()));
}
//	////////////////////////////////////////////////////////////////////////////
...
FileSizeType OSFileMMap::GetMaxMapLength()
{
	return(static_cast<FileSizeType>(0x7FFFFFFF -
		MLB::Utility::GetPageAllocGranularitySize()));
}
//	////////////////////////////////////////////////////////////////////////////
...
int TEST_OSFile(int return_code)
{
	...
	std::string tmp_file_name("./OSFile.TestFile." +
		MLB::Utility::UniqueId().ToString() + ".txt");

	try {
		FileSizeType alloc_gran = MLB::Utility::GetPageAllocGranularitySize();
		OSFile tmp_file_1;
		...
		tmp_file_1.Close();
		tmp_file_2.Close();
		MLB::Utility::RemoveFile(tmp_file_name);
	}
	catch (const std::exception &except) {
