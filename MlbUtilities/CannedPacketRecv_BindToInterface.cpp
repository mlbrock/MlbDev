...
void CannedPacketRecv::RunInternal()
{
...

	{
		...
		if (interface_name == NULL)
			std::cout << "not found, will not use setsockopt(SO_BINDTODEVICE)." <<
				std::endl;
		else {
			std::cout << "was set to '" << interface_name << "'." << std::endl;
			if (::setsockopt(tmp_recv_handler_list.back()->GetSocket().native(),
				SOL_SOCKET, SO_BINDTODEVICE, interface_name,
