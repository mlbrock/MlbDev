...
MLB_BOOSTX_SERIAL_DECL_Serialize(MLB::Utility::WaitProcessInfo)

	// Force instantiation of the Serialize{Load|Save}{Binary|Text|Xml} funcs.
MLB_BOOSTX_SerializeLoadSave(MLB::Utility::WaitProcessInfo)
...
template <typename SArchive>
	void serialize(SArchive &s_archive, MLB::Utility::WaitProcessInfo &datum,
		const unsigned int)
{
	s_archive
		& boost::serialization::make_nvp("processId",
			datum.process_id_)
		& boost::serialization::make_nvp("processExitCode",
...
int main()
{
	...
	std::cout <<
		"Test routine for Boost Serialization of class 'WaitProcessInfo'" <<
		std::endl;
	std::cout <<
		"---- ------- --- ----- ------------- -- ----- -----------------" <<
		std::endl;
	...
	try {
		WaitProcessInfo test_datum;
		TEST_Serialization(test_datum, "WaitProcessInfo");
	}
